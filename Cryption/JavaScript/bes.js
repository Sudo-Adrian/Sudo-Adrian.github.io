...
function l2n(l){
  ...
  for (let i of l.split("")){
    to_return.push(letters.indexOf(i)+1)
  }
  return to_return.join("")
}
...
function encrypt(str, password){
    ...
    let pw = text2binary(hash(password)).split(" ")
    let text = text2binary(str).split(" ") 
    let to_return = []
    for (let i of text){
      for (let x of pw){
        to_return.push(binary2hex(xor(i, x)))
      }
    }
  	return "*" + to_return.join("")
}
...
function decrypt(cipher, password){
    ...
    let pw = text2binary(hash(password)).split(" ")
    let str = hex2binary(l2n(cipher)).split("")
    let to_return = []
    for (let i of pw){
      to_return.push(xor(i, str))
    }

    return binary2text(to_return.join(" "))
